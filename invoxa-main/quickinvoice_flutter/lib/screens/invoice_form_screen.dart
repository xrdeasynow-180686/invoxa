...
class _InvoiceFormScreenState extends State<InvoiceFormScreen> {
  ...
  // Default to a single "Service Work" line — the most common tradie item.
  final List<Item> _items = [Item(name: 'Service Work')];
  ...
  Future<void> _loadLastClient() async {
    final last = await LastClientStorage.get();
    if (!mounted || last.isEmpty || _clientName.text.isNotEmpty) return;
    setState(() => _clientName.text = last);
  }
  ...
  void _addQuickItem(String name, double price) {
    setState(() {
      final onlyDefault = _items.length == 1 &&
          (_items.first.name.trim().isEmpty ||
              (_items.first.name == 'Service Work' &&
                  _items.first.price == 0.0));
      if (onlyDefault) {
        // Replace the unmodified default line so the invoice stays tidy.
        _items[0] = Item(name: name, quantity: 1, price: price);
      } else {
        // Append so nothing the tradie already typed gets lost.
        _items.add(Item(name: name, quantity: 1, price: price));
      }
    });
  ...
  Future<void> _generateAndShare() async {
    ...
    try {
      ...
      // Remember this client for next time (Quick Invoice auto-fill).
                  ...
                  Expanded(
                    child: OutlinedButton.icon(
                      style: OutlinedButton.styleFrom(
                        foregroundColor: const Color(0xFF0E1F4D),
                        side: const BorderSide(
                            color: Color(0xFF1D2D8C), width: 1.2),
                        shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(10)),
                      ),
                      onPressed: () =>
                          _addQuickItem('Call-out Fee', 80),
                      icon: const Icon(Icons.directions_car_filled, size: 18),
                      label: const Text('Call-out Fee'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(

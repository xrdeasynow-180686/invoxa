...
class HistoryStorage {
  ...
  /// Toggles or sets the payment status of a stored invoice.
  /// Returns the updated item, or `null` if no matching invoice was found.
  static Future<InvoiceHistoryItem?> updateStatus(
      String invoiceNumber, InvoiceStatus status) async {
    final list = await getAll();
  ...
  static Future<void> _persist(List<InvoiceHistoryItem> list) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(
